...
public class SuggestionDao {
    ...
    public List<String> getSuggestions(int userId, int limit) {
        return jdbcTemplate.query(
                "SELECT suggest FROM suggestion WHERE user_id = ? ORDER BY weight DESC LIMIT " + limit,
    ...
    public void addSuggest(int userId, String suggest) {
        jdbcTemplate.update(
                "INSERT INTO suggestion(suggest, user_id) VALUES (?, ?) ON CONFLICT (user_id, suggest) DO UPDATE SET weight = weight + 0.1",
                ps -> {
