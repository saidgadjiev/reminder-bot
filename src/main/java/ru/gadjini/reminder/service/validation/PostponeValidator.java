...
public class PostponeValidator implements Validator {
    ...
    public void validate(ValidationContext validationContext) {
        if (validationContext.time().isFixedTime()) {
            ...
            throw new UserException(localisationService.getCurrentLocaleMessage(MessagesProperties.MESSAGE_REMINDER_BAD_TIME_FORMAT, validationContext.reminder().getReceiver().getLocale()));
        }
    }
    ...
    private void validate(Reminder reminder, OffsetTime offsetTime) {
        ...
        if (!reminder.getRemindAt().hasTime() && (offsetTime.getHours() != 0 || offsetTime.getMinutes() != 0)) {
            throw new UserException(localisationService.getCurrentLocaleMessage(MessagesProperties.MESSAGE_REMINDER_POSTPONE_BAD_TIME_REMINDER_WITHOUT_TIME, reminder.getReceiver().getLocale()));
        }
    }
    ...
    private void validate(Reminder reminder, FixedTime fixedTime) {
        ...
        if (!reminder.getRemindAt().hasTime()) {
            ...
            if (!fixedTime.hasTime()) {
                throw new UserException(localisationService.getCurrentLocaleMessage(MessagesProperties.MESSAGE_REMINDER_POSTPONE_BAD_TIME_REMINDER_WITH_TIME, reminder.getReceiver().getLocale()));
            }
