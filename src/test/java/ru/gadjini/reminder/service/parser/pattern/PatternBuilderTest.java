...
class PatternBuilderTest {
    ...
    @Test
    public void test1() {
        Pattern p1 = Pattern.compile("((((?<minutes>\\d+)мин)?( )?(((?<hours>\\d+)ч)|(час))?( )?)|(?<everyminute>минуту)|(((?<hour>2[0-3]|[01]?[0-9]):(?<minute>[0-5]?[0-9]))?( )?(в )?((?<dayofweek>понедельник[а]?|вторник[а]?|сред[ыу]?|четверг[а]?|пятниц[ыу]?|суббот[ыу]?|воскресень[яе]?|пн|вт|ср|чт|пт|сб|вс)|((?<everyday>день)|((?<days>\\d+)дня))))) кажд[а-я]{0,2}");
