...
public class Patterns {
    ...
    public static final Pattern REPEAT_TIME_PATTERN = Pattern.compile("((\\b(?<hour>2[0-3]|[01]?[0-9])(:(?<minute>[0-5]?[0-9]))?\\b ?)(в ?)?)?(((((минут[ы]?|мин) )(?<prefixminutes>\\d+)|(?<suffixminutes>\\d+)(минут[ы]?|мин)( )?)?(( )?(((час[а-я]{1,2}|ч) )(?<prefixhours>\\d+)|(?<suffixhours>\\d+)(час[а-я]{1,2}|ч)( )?))?(( )?(((дн[а-я]{1,2}|д) )(?<prefixdays>\\d+)|(?<suffixdays>\\d+)(дн[а-я]{1,2}|д)))?)|(?<onehour>час)|(?<oneday>день)|(((числа )?(?<everymonthday>\\d+)(числа)?) ((?<onemonth>месяц)|(((месяц[а-я]{1,2}|м) )(?<prefixmonths>\\d+)|(?<suffixmonths>\\d+)(месяц[а-я]{1,2}|м))))|((?<monthword>января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря) (?<day>\\d+)( (?<oneyear>год))?)|(?<oneminute>минуту)|(?<dayofweek>понедельник[а]?|вторник[а]?|сред[ыу]?|четверг[а]?|пятниц[ыу]?|суббот[ыу]?|воскресень[яе]?|пн|вт|ср|чт|пт|сб|вс)) кажд[а-я]{1,2}");
